...
#define MAX 2048
    ...
    { // Taken from book "Hackers Delight"  / Aus dem Buch "Hackers Delight"
        x = x - ((x >> 1) & 0x55555555);
    ...
    int solve()
    {
            ...
            for (y = 0; y < MAX; y++)
            {
                for (x = 0; x < MAX; x++)
                {
                    if ((mask[x][reduction]) && (mask[y][reduction]))
                    {
                        ...
                        { // Do not allow complete removal (problem if all terms are 1)  /  Komplette aufhebung nicht zulassen (sonst problem, wenn alle Terme = 1)
                            ...
                            {                                 // Simplification only possible if 1 bit differs  /  Vereinfachung nur möglich, wenn 1 anderst ist
        ...
        { // for all minterms  /  alle Minterme durchgehen
            ...
            if (mask[y][0])
            {
                ...
                { // for all prime implicants  /  alle Primimplikanten durchgehen
                    if (primmask[x])
                    {
        ...
        for (z = 0; z < prim_count; z++)
        {
            if (primmask[z])
            {
                ...
                {                      // && (rwprim[z] == TRUE))
                    ...
                    for (y = 0; y < pos; y++)
                    { // for all possibilities  /  alle Möglichkeiten durchgehen
                        res = 0;
                        for (x = 0; x < prim_count; x++)
                        {
                            if ((wprim[x] == TRUE) || (nwprim[x] == TRUE))
                            { // essential prime implicant or marked as required  /  wesentlicher Primimplikant oder als benötigt markiert
                                if ((y & primmask[x]) == (prim[x] & primmask[x]))
                                { // All bits must be 1  /  Es müssen alle Bits auf einmal auf 1 sein (da And-Verknüpfung)
                                    res = 1;
                                    break;
                                }
                            }
